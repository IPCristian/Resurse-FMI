...
string BFS(int S,string Cuv)                // BFS realizat dupa modelul de pseudocod oferit in cadrul celui de-al doilea laborator (cu diferite modificari)
{
    ...
    while (!Coada.empty())
    {
        ...
        while (ElementCrt.adancime < Drum.size())                 // Aceste lucru se intampla in cazul in care am ajuns intr-un punct
        ...
        if (ElementCrt.indice == Cuv.size())
        {
            for (int k=0;k<StFin.size();k++)
                if (ElementCrt.stare == StFin[k])
                {
                    Drum.push(-1);
                    return "DA";
                }
        ...
        for (int i=0;i<L[ElementCrt.stare].size();i++)
        {
            ...
            if (L[ElementCrt.stare][i].litera == '$')
            {
                elcoada b;
                b.stare = L[ElementCrt.stare][i].destinatie;
                b.indice = ElementCrt.indice;
                b.adancime = ElementCrt.adancime + 1;
                Coada.push(b);
            }
...
int main()
{
    ...
    for (int r=0;r<NrCuv;r++)
    {
        ...
        if (Drum.top() == -1)
        {
            ...
            cout<<"Cuvant: "<<CuvCrt<<endl<<"Rezolvare: ";
