...
hEval (HIf cond sttrue stfalse) env = hif (hEval cond env) (hEval sttrue env) (hEval stfalse env)
    where
        ...
        hif _ _ _ = error "Tipuri de date incompatibile"
hEval (exp1 :+: exp2) env = hadd (hEval exp1 env) (hEval exp2 env)
    where
        hadd (VInt i1) (VInt i2) = VInt $ i1 + i2
        hadd _ _ = error "Tipuri de date incompatibile"
hEval (exp1 :*: exp2) env = hmult (hEval exp1 env) (hEval exp2 env)
    where
        hmult (VInt i1) (VInt i2) = VInt $ i1 * i2
        hmult _ _ = error "Tipuri de date incompatibile"
...
hEval (f :$: g) env = happ (hEval f env) (hEval g env)
    where
        happ (VFun f) v = f v
        happ _ _ = error "f nu reprezinta o functie eligibila"
...
h2 = (HLet "x" (HLit 1) (HLet "y" (HLit 2) (HLet "z" (HLit 3) ((HVar "x") :+: (HVar "y") :*: (HVar "z")))))

h3 = (HLet "x" (HLit 5) (HLet "y" (HLit 15) (HIf (HVar "x" :==: HVar "y") (HTrue) (HFalse))))

-- Tema:
-- Adaugati semantica pt :*: - Done
-- Rulati expresiile h0 si h1 si adaugati inca 2 care sa foloseasca celelalte tipuri - Done
-- Modificati programul ai in loc de VError sa se arunce o  exceptie cu un mesaj specific - Done
-- utilizati functia error din Haskell
